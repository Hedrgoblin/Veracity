...
export default class TrainMapScene extends Phaser.Scene {
  constructor() {
    super({ key: 'TrainMapScene' });
  }
  ...
  preload() {
    if (!this.textures.exists('train_map_display')) {
      this.load.image('train_map_display', `${BASE}assets/images/backgrounds/puzzle_train_map_01.png`);
    }
    if (!this.textures.exists('train_map_debug')) {
      this.load.image('train_map_debug', `${BASE}assets/images/backgrounds/puzzle_train_map_01_debug.png`);
    }
  }
  ...
  create() {
    ...
    // Display map image scaled to fit
    this.mazeImage = this.add.image(width / 2, height / 2, 'train_map_display').setOrigin(0.5);
  ...
  buildDebugPixelMap() {
    const src = this.textures.get('train_map_debug').getSourceImage();
  ...
  triggerWin() {
    ...
    this.add.text(width / 2, height / 2 - 40, 'You followed the train!', {
