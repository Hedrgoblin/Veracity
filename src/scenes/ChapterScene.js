...
export default class ChapterScene extends Phaser.Scene {
  ...
  setupPuzzleJumpMenu() {
    // DEBUG MENU - Jump directly to puzzles/mini-games
    const width = this.cameras.main.width;
    const buttonWidth = 60;
    const buttonHeight = 35;
    const padding = 15;

    // Position to the left of JUMP button
    const buttonX = width - padding - 80 - 10 - buttonWidth / 2; // JUMP is 80px wide + 10px gap
    const buttonY = padding + buttonHeight / 2;

    this.puzzleJumpContainer = this.add.container(buttonX, buttonY);
    this.puzzleJumpContainer.setDepth(1000);

    // JP button
    const jpButton = this.add.rectangle(0, 0, buttonWidth, buttonHeight, 0x4a90e2);
    jpButton.setStrokeStyle(2, 0xffd700);
    jpButton.setInteractive({ useHandCursor: true });

    const jpText = this.add.text(0, 0, 'JP', {
      fontSize: '14px',
      fontFamily: 'Courier New',
      color: '#ffffff',
      fontStyle: 'bold'
    ...
    this.puzzleJumpContainer.add([jpButton, jpText]);

    // Define available puzzles
    const puzzles = [
      { name: 'Gear Puzzle', type: 'connection', data: {
        type: 'connection',
        title: 'The Study Lock',
        instructions: 'Connect the gears to unlock the door.',
        points: [
          { id: 'gear1', x: -200, y: -100, pair: 'gear2' },
          { id: 'gear2', x: 200, y: -100, pair: 'gear1' },
          { id: 'spring1', x: -200, y: 100, pair: 'spring2' },
      ...
      { name: 'Note Puzzle', type: 'puzzle_pieces', data: {
        type: 'puzzle_pieces',
        title: 'Piece Together the Note',
        instructions: 'Arrange the torn pieces to read the full message.',
        pieceCount: 7,
        finalImage: 'note_dear_vera'
      }},
      { name: 'Safe', type: 'sequence', data: {
        type: 'sequence',
        title: 'The Secret Panel',
        instructions: 'Follow the sequence to open the hidden compartment.',
        steps: [
    ...
    // Dropdown menu
    const dropdownHeight = 30 + (puzzles.length * 30);
    const dropdownTop = buttonY + buttonHeight / 2 + 5;

    this.puzzleDropdown = this.add.container(buttonX, dropdownTop + dropdownHeight / 2);
    this.puzzleDropdown.setDepth(1001);
    this.puzzleDropdown.setAlpha(0);
    this.puzzleDropdown.setVisible(false);

    const dropdownBg = this.add.rectangle(0, 0, 180, dropdownHeight, 0x000000, 0.95);
    dropdownBg.setStrokeStyle(2, 0x4a90e2);
    this.puzzleDropdown.add(dropdownBg);

    // Add puzzle buttons
    const buttonSpacing = 30;
    const startY = -(dropdownHeight / 2) + 20;

    puzzles.forEach((puzzle, index) => {
      const yPos = startY + index * buttonSpacing;
      const puzzleButton = this.add.rectangle(0, yPos, 160, 26, 0x333333);
      puzzleButton.setInteractive({ useHandCursor: true });

      const puzzleText = this.add.text(0, yPos, puzzle.name, {
        fontSize: '11px',
        fontFamily: 'Courier New',
        color: '#ffffff'
      }).setOrigin(0.5);
      ...
      puzzleButton.on('pointerover', () => {
        puzzleButton.setFillStyle(0x4a90e2);
      });
      puzzleButton.on('pointerout', () => {
        puzzleButton.setFillStyle(0x333333);
      });

      // Click to launch puzzle
      puzzleButton.on('pointerdown', () => {
        console.log(`Launching ${puzzle.name}`);
        this.launchPuzzleDirectly(puzzle.data);
      });

      this.puzzleDropdown.add([puzzleButton, puzzleText]);
    });

    // Toggle dropdown
    let isPuzzleOpen = false;
    jpButton.on('pointerdown', () => {
      isPuzzleOpen = !isPuzzleOpen;
      this.puzzleDropdown.setVisible(isPuzzleOpen);
      this.tweens.add({
        targets: this.puzzleDropdown,
        alpha: isPuzzleOpen ? 1 : 0,
        duration: 200
      });
    });
  }

  launchPuzzleDirectly(puzzleData) {
    // Close dropdown
    this.puzzleDropdown.setVisible(false);
    this.puzzleDropdown.setAlpha(0);
